...
export class AuthService {
  ...
  async refresh(refreshToken: string, meta?: { userAgent?: string; ip?: string }) {
    try {
      const payload = this.jwt.verify(refreshToken, { secret: this.config.get<string>('JWT_REFRESH_SECRET') }) as any;
      ...
      if (record.revokedAt || record.expiresAt <= now) {
        await repo.updateMany({ where: { familyId: record.familyId, revokedAt: null }, data: { revokedAt: now } });
      ...
      const next = await this.issueTokensWithRefreshRecord(user.id, user.email, record.familyId, meta);
      await repo.update({ where: { jti }, data: { revokedAt: now, replacedById: (next as any).refreshJti } });
      return next;
    } catch {
      throw new UnauthorizedException('Invalid refresh');
    }
  ...
  private async issueTokensWithRefreshRecord(sub: string, email: string, familyId?: string, meta?: { userAgent?: string; ip?: string }) {
    ...
    const expiresAt = this.computeExpiryFromNow(this.config.get<string>('JWT_REFRESH_EXPIRES_IN') || '30d');
